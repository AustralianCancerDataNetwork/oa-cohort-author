            ...
            summary_sections=(
                DetailSection(
                    ...
                    rows=(
                        ...
                        DetailRow("Benchmark", _benchmark_summary(entity)),
                        DetailRow("Numerator window", _window_summary(entity, "numerator")),
                        DetailRow("Denominator window", _window_summary(entity, "denominator")),
                    ),
                ),
            ),
            secondary_sections=(
                DetailSection(
                    title="Numerator",
                    rows=(
                        DetailRow("Label", entity.numerator_label or "-"),
                        DetailRow(
                            "Measure",
                        ...
                        *_measure_context_rows(entity.numerator_measure),
                    ),
                ),
                DetailSection(
                    title="Denominator",
                    rows=(
                        DetailRow("Label", entity.denominator_label or "-"),
                        DetailRow(
                            "Measure",
                        ...
                        *_measure_context_rows(entity.denominator_measure),
                    ),
                ),
                DetailSection(title="Reports", rows=report_rows or (DetailRow("Reports", "-"),)),
            ),
            hide_relationships=True,
        )
    if kind is EntityKind.dash_cohort:
    ...
    if kind is EntityKind.dash_cohort_def:
        linked_cohorts = tuple(
            DetailRow(
                ...
                f"{len(cohort.definitions)} definition{'s' if len(cohort.definitions) != 1 else ''}",
                link=_detail_link(EntityKind.dash_cohort, cohort.dash_cohort_id, cohort.dash_cohort_name),
            )
            for cohort in sorted(entity.dash_cohort_objects, key=lambda item: item.dash_cohort_name)
        )
        linked_reports = tuple(
            DetailRow(
                report_map.report.report_name,
                report_map.report.report_short_name or "-",
                link=_detail_link(EntityKind.report, report_map.report.report_id, report_map.report.report_name),
            )
            for cohort in sorted(entity.dash_cohort_objects, key=lambda item: item.dash_cohort_name)
            for report_map in sorted(cohort.in_reports, key=lambda item: item.report.report_name if item.report is not None else "")
            if report_map.report is not None
        )
        return TailoredDetailView(
            summary_sections=(
                DetailSection(
                    ...
                    rows=(
                        ...
                        DetailRow("Measure shape", _measure_shape(entity.dash_cohort_measure)),
                    ),
                ),
            ),
            secondary_sections=(
                DetailSection(
                    title="Measure context",
                    rows=_measure_context_rows(entity.dash_cohort_measure),
                ),
                DetailSection(
                    title="Linked cohorts",
                    rows=linked_cohorts or (DetailRow("Cohorts", "-"),),
                ),
                DetailSection(title="Reports", rows=linked_reports or (DetailRow("Reports", "-"),)),
            ),
            hide_relationships=True,
        )
    if kind is EntityKind.measure:
        rows = [
            ...
            DetailRow("Shape", _measure_shape(entity)),
            DetailRow("Episode override", "yes" if entity.person_ep_override else "no"),
            DetailRow("Used by", str(usage.inbound_count)),
        ]
        secondary_sections: list[DetailSection] = []
        definition_rows = _measure_context_rows(entity)
        if definition_rows:
            secondary_sections.append(DetailSection(title="Definition", rows=definition_rows))
        parent_rows = tuple(
            DetailRow(
                parent_link.parent.name,
                parent_link.parent.combination.value,
                link=_detail_link(EntityKind.measure, parent_link.parent.measure_id, parent_link.parent.name),
            )
            for parent_link in entity.parent_links
            if parent_link.parent is not None
        )
        if parent_rows:
            secondary_sections.append(DetailSection(title="Parent measures", rows=parent_rows))
        child_rows = tuple(
            DetailRow(
                child.name,
        ...
        if child_rows:
            secondary_sections.append(DetailSection(title="Child measures", rows=child_rows))
        inbound_rows = _usage_rows(usage.inbound)
        if inbound_rows:
            secondary_sections.append(DetailSection(title="Used by", rows=inbound_rows))
        return TailoredDetailView(
            summary_sections=(DetailSection(title="Summary", rows=tuple(rows)),),
            secondary_sections=tuple(secondary_sections),
            hide_relationships=True,
        )
    return None


def _window_summary(entity: Indicator, side: str) -> str:
    prior = getattr(entity, f"{side}_max_days_prior", None)
    post = getattr(entity, f"{side}_max_days_post", None)
    if prior is None and post is None:
        return "Unrestricted"
    return f"-{prior if prior is not None else '∞'} / +{post if post is not None else '∞'} days"


def _benchmark_summary(entity: Indicator) -> str:
    if entity.benchmark is None:
        return "-"
...
def _measure_context_rows(measure: Measure) -> tuple[DetailRow, ...]:
    rows: list[DetailRow] = []
    if measure.measure_id == 0:
        rows.append(DetailRow("Scope", "Full report cohort"))
        return tuple(rows)
    if measure.subquery is not None:
        rows.append(
            DetailRow(
        ...
        rows.append(DetailRow("Temporality", measure.subquery.temporality.value))
    if measure.children:
        rows.append(DetailRow("Child measures", str(len(measure.children))))
    return tuple(rows)


def _usage_rows(entries: tuple[str, ...]) -> tuple[DetailRow, ...]:
    rows: list[DetailRow] = []
    for entry in entries:
        kind_str, entity_id, label = _parse_usage_entry(entry)
        if kind_str is None or entity_id is None or label is None:
            rows.append(DetailRow("Usage", entry))
            continue
        try:
            kind = EntityKind(kind_str)
        except ValueError:
            rows.append(DetailRow(kind_str, label))
            continue
        rows.append(
            DetailRow(
                kind.value.replace("_", " "),
                label,
                link=_detail_link(kind, entity_id, label),
            )
        )
    return tuple(rows)


def _parse_usage_entry(entry: str) -> tuple[str | None, int | None, str | None]:
    parts = entry.split(":")
    if len(parts) < 3:
